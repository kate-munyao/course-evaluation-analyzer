"""
Sentiment Analysis for Course Evaluations - FIXED VERSION
...
print(f"\n✅ Best: {best_model_name} ({results[best_model_name]:.3f})")
...
ax.bar(x - width, sentiment_by_topic['positive'], width, 
       label='Positive', color='green')
ax.bar(x, sentiment_by_topic['neutral'], width, 
       label='Neutral', color='orange')
ax.bar(x + width, sentiment_by_topic['negative'], width, 
       label='Negative', color='red')

ax.set_xlabel('Topic', fontsize=12, fontweight='bold')
ax.set_ylabel('Count', fontsize=12, fontweight='bold')
ax.set_title('Sentiment by Topic', fontsize=14, fontweight='bold')
ax.set_xticks(x)
